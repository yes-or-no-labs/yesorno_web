...
async function getPointsLeaderboard() {
    try {
        const res = await api.getPointsLeaderboard()
        if(res.success){
            state.dataList = res.obj
        }
    } catch (error) {
...
<template>
  <div class="w-full relative !pb-[100px] sm:min-h-screen">
    ...
    <div
      class="relative 2xl:!px-[50px] xl:!px-[40px] lg:!px-[30px] sm:!px-[20px] !px-[10px] sm:!mt-[100px]"
    >
      ...
      <div class="!pt-[40px]">
        ...
        <v-window v-model="state.currentTab">
          <v-window-item value="1">
            <div
              class="w-full !pb-[30px] border border-solid !border-[rgba(255,255,255,.5)] rounded-[20px] !mt-[20px] sm:h-[800px] h-[500px] overflow-y-auto"
            >
              <div class="flex items-center !px-[40px] !py-[20px]">
                <div
                  class="lg:flex-[0.3] sm:flex-[0.5] flex-1 text-left text-[#7A7A7A] text-[16px] font-[600] leading-[16px]"
                >
                  Rank
                </div>
                <div
                  class="flex-1 sm:!text-left text-center text-[#7A7A7A] text-[16px] font-[600] leading-[16px]"
                >
                  Address
                </div>
                <div class="flex-1 text-right text-[#7A7A7A] text-[16px] font-[600] leading-[16px]">
                  Points
                </div>
              </div>
              <div
                class="flex items-center !py-[20px] !px-[40px]"
                :style="filterStyle(index)"
                v-for="(item, index) in state.dataList"
                :key="index"
              >
                <div
                  class="lg:flex-[0.3] sm:flex-[0.5] flex-1 flex items-center lg:gap-[30px] gap-[10px] text-[#9F9F9F] text-[24px] font-[600] leading-[24px]"
                >
                  <div
                    class="text-[#fff] text-[16px] leading-[22px] text-center w-[20px]"
                  ...
                  />
                  <img
                    src="@/assets/img/no2.png"
                    class="w-[18px] h-[18px]"
                    v-show="index + 1 == 2"
                  />
                  <img
                    src="@/assets/img/no3.png"
                    class="w-[18px] h-[18px]"
                    v-show="index + 1 == 3"
                  />
                </div>
