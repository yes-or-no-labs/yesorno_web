...
<template>
    <div class="fixed bottom-0 left-0 lg:hidden flex items-center justify-between border-t border-solid border-[#FFFFFF80] w-full bg-[#000] z-10">
      <div class="flex flex-col items-center flex-1 !py-[10px]" v-for="(item,index) in tabList" :key="index" v-ripple @click="$router.push(item.path)">
        <img :src="currenRoutePath===item.path?item.selectIcon:item.icon" class="w-[18px] h-[18px]" alt="">
        <div class="text-[#A7A7A7] text-[14px]" :style="currenRoutePath===item.path?'color:#6DDD25':''">{{ item.name }}</div>
      </div>
    </div>
</template>